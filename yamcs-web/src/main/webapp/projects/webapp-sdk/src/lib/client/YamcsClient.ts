...
export default class YamcsClient implements HttpHandler {
  ...
  async triggerParameterRebuild(instance: string, options: RebuildParametersOptions = {}) {
    const body = JSON.stringify(options);
    const url = `${this.apiUrl}/archive/${instance}/parameterArchive:rebuild`;
    return await this.doFetch(url, {
      body,
      method: 'POST',
    });
  ...
  // Key Management
  async getActiveKeyId(instance: string, options: ActiveKeyRequest) {
    const url = `${this.apiUrl}/keymanagement/${instance}`;
    const response = await this.doFetch(url + this.queryString(options));
    return await response.json() as KeyInfo;
  }
  ...
  async getMissionDatabaseVersion(instance: string) {
